...
namespace tlr
{
	...
	int minValue = 200;
	int expected_lights = 2;
	int maxGreenHDist = 30;
	int maxRedHDist = 7;
	int maxYellowHDist = 20;
	...
	cv::Mat iTrafficLight(const cv::Mat & frame)
	{
				...
				if(C >= minCircularity)
				{
					...
					if(avg_S >= minSaturation)
					{
						...
						if(avg_V >= minValue)
						{
							...
							if(std::abs(avg_H-60) <= maxGreenHDist)
								light_color = cv::Scalar(0, 255, 0);
							/*else if(std::abs(avg_H-30) <= maxYellowHDist)
								light_color = cv::Scalar(0, 255, 255);
							else
								light_color = cv::Scalar(0, 0, 255);*/
							else if(avg_H <= maxRedHDist || (180-avg_H) <= maxRedHDist)
								light_color = cv::Scalar(0, 0, 255);
							else
								light_color = cv::Scalar(0, 255, 255);
		...
		if(n_detected_objects != expected_lights)
		{
