...
namespace
{
	...
	void CannyEdgeDetectionCallback(int pos, void* userdata)
	{
		if(Canny_sigmaX10 == 0)
			return;
		...
		cv::Mat img_gaussian;
		int n = ucas::round(6 * Canny_sigmaX10/10.0);
		if(n%2 == 0)
			n++;
		cv::GaussianBlur(img, img_gaussian, cv::Size(n, n), Canny_sigmaX10/10.0);

		cv::Mat result;
		cv::Canny(img_gaussian, result, Canny_thresh, Canny_thresh*3);
		cv::imshow(win_name_3, result);
	}
...
int main() 
{
	try
	{
		img = cv::imread(std::string(EXAMPLE_IMAGES_PATH) + "/retina.png");
		...
		cv::namedWindow(win_name_3);
		cv::createTrackbar("sigma_x10", win_name_3, &Canny_sigmaX10, 100,  CannyEdgeDetectionCallback);
		cv::createTrackbar("thresh", win_name_3, &Canny_thresh, 200,  CannyEdgeDetectionCallback);

		// start the UIs
		gradientEdgeDetectionCallback(0, 0);
		LoGDetectionCallback(0, 0);
		CannyEdgeDetectionCallback(0,0);
