...
namespace Paint
{
	...
	void updatePaint(int event, int x, int y, int, void* userdata)
	{
		...
		else if (event == cv::EVENT_MOUSEMOVE && drawing)
		{
			...
			if (draw_FG == false)
			{
				cv::line(img_copy, prevPos, cv::Point(x, y), cv::Scalar(255, 0, 0), 12, CV_AA);
				cv::line(seeds_BG, prevPos, cv::Point(x, y), cv::Scalar(255), 12);
			}
			else
			{
				cv::line(img_copy, prevPos, cv::Point(x, y), cv::Scalar(0, 0, 255), 12, CV_AA);
...
int main()
{
	...
	{
		// load image
		Paint::img = cv::imread(std::string(EXAMPLE_IMAGES_PATH) + "/for_canny.png");
		if (!Paint::img.data)
		...
		std::vector<int> hist_FG = ucas::histogram(Paint::img, Paint::seeds_FG);
		std::vector<int> hist_BG = ucas::histogram(Paint::img, Paint::seeds_BG);
